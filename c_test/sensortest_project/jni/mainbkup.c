...
int main(int argv, char *argc[]){
  ...
  fp = fopen("/data/local/tmp/test2.txt", "w+");
  fprintf(fp, "Test fprintf...\n");
  fputs("Test fputs..\n", fp);
  fflush(fp);
  
  char file_write_buffer[128*1000];
  int written_lines = 0;
  ...
  for (;;) {
      ...
      val_x = sEvent.acceleration.x;
			val_y = sEvent.acceleration.y;
			val_z = sEvent.acceleration.z;
			
      sprintf(line, "[%s] : X=%f Y=%f Z=%f\n", str, val_x, val_y, val_z );
      printf("%s", line);
			strcat(file_write_buffer, line);
			written_lines++;
			if (written_lines>1000){
				printf("Flush");
